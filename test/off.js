...
describe('#off', () => {
    ...
    it('should remove if phase does match', () => {
        ...
        eh.countCallbacks('a.b', {phase: EventHub.PHASES.BUBBLING}).should.equal(1);
        eh.countCallbacks('a.b.c', {phase: EventHub.PHASES.BUBBLING}).should.equal(2);
    });

    it('should recursively remove callbacks', () => {
        eh.off('a', cbs.cb1, {phase: EventHub.PHASES.BUBBLING, traverse: true});
        eh.countCallbacks('a.b', {phase: EventHub.PHASES.BUBBLING}).should.equal(1);
        eh.countCallbacks('a.b.c', {phase: EventHub.PHASES.BUBBLING}).should.equal(1);
    })

    it('should only remove a `isOne` callback', () => {
        eh.off('a', cbs.cb2, { isOne: true, traverse: true});
        eh.countCallbacks('a.b', {traverse: true}).should.equal(2);
        eh.countCallbacks('a.b.c').should.equal(2);
    });

    it('should remove without a callback given (bubbling)', () => {
        eh.countCallbacks('a.b.c', {traverse: true}).should.equal(3);
        eh.off('a', {traverse: true, phase: EventHub.PHASES.BUBBLING});
        eh.countCallbacks('a.b.c', {traverse: true}).should.equal(0);
    });

    it('should remove without a callback given', () => {
        eh.countCallbacks('a', {traverse: true}).should.equal(2);
        eh.off('a', {traverse: true});
        eh.countCallbacks('a', {traverse: true}).should.equal(0);
    });
